...
void graphics::Texture::makePowerOfTwo() {
    ...
    int u2 = 1;
    while (u2 < m_width) u2 *= 2;
    int v2 = 1;
    while (v2 < m_height) v2 *= 2;
    ...
    for (int i = 0; i < v2; i++)
        for (int j = 0; j < u2; j++) {
            ...
            for (int c = 0; c < 4; c++) {
                float valx1 = (1.0f - sx) * m_buffer[4 * (y_L * m_width + x_L) + c] + sx * m_buffer[
                                  4 * (y_L * m_width + x_U) + c];
                float valx2 = (1.0f - sx) * m_buffer[4 * (y_U * m_width + x_L) + c] + sx * m_buffer[
                                  4 * (y_U * m_width + x_U) + c];
...
bool graphics::Texture::buildGLTexture() {
    if (m_customBuildFunction) {
        m_customBuildFunction(); // Call the custom build function
    } else {
